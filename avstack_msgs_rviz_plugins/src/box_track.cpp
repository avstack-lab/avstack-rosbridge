...
#include <avstack_msgs_rviz_plugins/box_track.hpp>
...
BoxTrackDisplay::BoxTrackDisplay()
{
...
void BoxTrackDisplay::onInitialize()
{
  ...
  topic_property_->setDescription("BoxTrack topic to subscribe to.");
...
void BoxTrackDisplay::processMessage(
  BoxTrack::ConstSharedPtr msg)
{
  latest_msg = msg;
  if (!only_edge_) {
    showBoxes(msg);
...
void BoxTrackDisplay::update(float wall_dt, float ros_dt)
{
  m_marker_common->update(wall_dt, ros_dt);
}

void BoxTrackDisplay::reset()
{
...
void BoxTrackDisplay::updateEdge()
{
  only_edge_ = only_edge_property_->getBool();
  if (only_edge_) {
...
void BoxTrackDisplay::updateLineWidth()
{
  line_width = line_width_property_->getFloat();
  if (latest_msg) {
...
void BoxTrackDisplay::updateAlpha()
{
  alpha = alpha_property_->getFloat();
  if (latest_msg) {
    processMessage(latest_msg);
...
PLUGINLIB_EXPORT_CLASS(rviz_plugins::BoxTrackDisplay, rviz_common::Display)